...
condition_text= f"""Please identify a {exp_info['condition']} event.
Can you name the event?
"""
condition_txt=TextStim(win,condition_text,pos=(0,0))
condition_txt.draw()
typed_text = ''
box=TextStim(win,typed_text, pos=(0,0))
box.draw()
win.flip()
wait(2)
event.clearEvents()

typed=TextStim(win,"Type a two word name for your event below")
typed.draw()
win.flip()
...
while not done:
    ...
    for key in keys:
        if key == 'backspace':
            ...
            typed_text += key
            box=TextStim(win,typed_text, pos=(0,-0.2))
        if key == 'return':
            done = True
            break
        typed.draw()
...
imagine=f"Please imagine your event {typed_text} for two minutes"
imagine_stim=TextStim(win,imagine)
imagine_stim.draw()
win.flip()
wait(120)
...
### TRIAL LOOP ROUTINE ###
offer_trial = pd.read_csv(offers[1])
offer_trial = offer_trial.sample(frac=1)
...
offer_trial.to_csv(f"sub-{exp_info['participant_nr']}_results_{exp_info['condition']}.csv")
